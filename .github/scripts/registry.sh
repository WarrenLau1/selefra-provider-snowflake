...
for f in $FOR; do
  ...
  windows_amd64=`cat selefra-provider-snowflake* | grep windows_amd64 | awk -F ' ' '{print $1}'`
  ...
  windows_arm64=`cat selefra-provider-snowflake* | grep windows_arm64 | awk -F ' ' '{print $1}'`
  ...
  sed "${sedi[@]}" "s#{{.CheckSumWindowsARM64}}#${windows_arm64}#g" provider/snowflake/$version/supplement.yaml
  sed "${sedi[@]}" "s#{{.CheckSumWindowsAMD64}}#${windows_amd64}#g" provider/snowflake/$version/supplement.yaml
...
if [[ "$VERSION" != "$version" ]]; then
  ...
  sed "${sedi[@]}" "s#{{.Introduction}}#A Selefra provider for Amazon Web Services (snowflake).#g" provider/template/metadata.yaml
